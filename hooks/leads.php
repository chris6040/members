<?php
	...
	*/
	...
	function leads_after_insert($data, $memberInfo, &$args){
		...
		@mail("chris@boost-property-profits.uk",
		/* message subject */
		"New order added",
		/* message contents */
		...
		"http://members.boost-property-profits.uk/db/orders_view.php?SelectedID=$recID",
		/* we must add sender email similar to this */
		"From: webmaster@boost-property-profits.uk"
		);
	...
	*/
	...
	function update_commission($data){
		...
		*/
		...
		$item = makeSafe($data['id']);

		/* get sum of all item transactions and update item balance */
		if($item){


		$item_balance = sqlValue("select estimated_value * 0.6 * 0.005 from leads where id={$item}");

		/* update the item balance with the value calculated above */

		sql("update leads set est_commission='{$item_balance}' where id={$item}", $eo);
