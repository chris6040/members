<?php
	...
	/**
	 * @file
	 * This file contains hook functions that get called when data operations are performed on 'partner' table. 
	...
	*/
	...
	function partner_init(&$options, $memberInfo, &$args){
	...
	*/
	...
	function partner_header($contentType, $memberInfo, &$args){
		$header='';
	...
	*/
	...
	function partner_footer($contentType, $memberInfo, &$args){
		$footer='';
	 ...
	 *     $data['company_name'], $data['title'], $data['fname'], $data['lname'], $data['mailname'], $data['email'], $data['phone'], $data['mobile'], $data['address_id'], $data['username'], $data['password'], $data['type1'], $data['type2'], $data['type3']
	...
	*/
	...
	function partner_before_insert(&$data, $memberInfo, &$args){
	...
	*/
	...
	function partner_after_insert($data, $memberInfo, &$args){
		...
		$sql = "INSERT INTO membership_users(memberID, passMD5, email, signupDate, groupID, isBanned, isApproved) 
	 ...
	 * Note: if a field is set as read-only or hidden in detail view, it can't be modified through $data. You should use a direct SQL statement instead.
	 * For this table, the array items are: 
	 *     $data['id'], $data['company_name'], $data['title'], $data['fname'], $data['lname'], $data['mailname'], $data['email'], $data['phone'], $data['mobile'], $data['address_id'], $data['username'], $data['password'], $data['type1'], $data['type2'], $data['type3']
	...
	*/
	...
	function partner_before_update(&$data, $memberInfo, &$args){
	...
	*/
	...
	function partner_after_update($data, $memberInfo, &$args){
	...
	*/
	...
	function partner_before_delete($selectedID, &$skipChecks, $memberInfo, &$args){
	...
	*/
	...
	function partner_after_delete($selectedID, $memberInfo, &$args){
	...
	*/
	...
	function partner_dv($selectedID, $memberInfo, &$html, &$args){
	...
	*/
	...
	function partner_csv($query, $memberInfo, &$args){
	...
	*/
	...
	function partner_batch_actions(&$args){
