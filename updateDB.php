<?php
	...
	if($thisMD5==$prevMD5){
		...
		// set up tables
		setupTable('leads', "create table if not exists `leads` (   `id` INT unsigned not null auto_increment , primary key (`id`), `name` VARCHAR(40) , `email` VARCHAR(40) , `phone` VARCHAR(20) , `prop_area` VARCHAR(10) , `tenanted` VARCHAR(4) , `notes` TEXT , `date_entered` DATE , `date_contacted` DATE , `est_value` VARCHAR(10) default '0' , `os_capital` VARCHAR(15) , `rental_income` DECIMAL(6,2) , `finance_costs` DOUBLE(7,2) , `other_costs` DOUBLE(6,2) , `income_tax_band` VARCHAR(3) , `referer` VARCHAR(40) , `est_commission` DOUBLE(6,2) ) CHARSET utf8", $silent, array( " ALTER TABLE `leads` CHANGE `income_tax_band` `income_tax_band` VARCHAR(40) "," ALTER TABLE `leads` CHANGE `income_tax_band` `income_tax_band` VARCHAR(3) "));
		setupTable('property', "create table if not exists `property` (   `id` INT unsigned not null auto_increment , primary key (`id`), `client_id` INT unsigned , `referer` INT unsigned , `address_id` INT unsigned , `contact_date` DATE , `purchase_date` DATE , `detachment` VARCHAR(40) , `type` VARCHAR(40) , `tenancy_type` VARCHAR(40) , `no_beds` TINYINT , `notes` TEXT ) CHARSET utf8", $silent);
		setupIndexes('property', array('client_id','referer','address_id'));
		...
		setupTable('client', "create table if not exists `client` (   `id` INT unsigned not null auto_increment , primary key (`id`), `company_name` VARCHAR(40) , `title` TINYINT unsigned , `fname` VARCHAR(40) , `lname` VARCHAR(40) , `mailname` VARCHAR(40) , `email` VARCHAR(80) , `phone` VARCHAR(40) , `mobile` VARCHAR(40) , `address_id` INT unsigned , `username` VARCHAR(40) , `password` VARCHAR(40) , `type1` INT unsigned , `type2` INT unsigned , `type3` INT unsigned , `commission_rate` DOUBLE(3,2) default '0.50' , `membergroupid` SMALLINT unsigned ) CHARSET utf8", $silent);
		setupIndexes('client', array('title','address_id','type1','type2','type3','membergroupid'));
